 ...
 *
 *  non-blocking feed() changes by hchunhui
 *******************************************************************************
 */
#include <string.h>
#include <stdlib.h>
...
#define LPC_STATE_STOPPED		4
	...
	int variant;
#ifdef LPC_UPSAMPLE
	short synth_buf[LPC_SAMPLES_PER_FRAME];
#endif
} LPC_SYNTH;
...
//------------------------------------------------------------------------------
static bool lpc_get_frame(LPC_SYNTH* s, LPC_FRAME* dst, LPC_FRAME* ref, int *__res)
{
	static int pitch_i;
	static int energy_i;
	static int repeat;
	...
	TRY(get_nbits(s, 1, &repeat)); // repeat
	TRY(get_nbits(s, 7, &pitch_i)); // pitch
	...
	if (repeat) {
		...
		TRY(get_nbits(s, 6, &i0));
		dst->k[0] = lpc_k1_tab[i0]; // K1
		TRY(get_nbits(s, 6, &i1));
		dst->k[1] = lpc_k2_tab[i1]; // K2
		TRY(get_nbits(s, 5, &i2));
		dst->k[2] = lpc_k3_tab[i2]; // K3
		TRY(get_nbits(s, 5, &i3));
		dst->k[3] = lpc_k4_tab[i3]; // K4
	}
	...
	if (0 == pitch_i) {
		...
		if (repeat) {
			...
			static int i4, i5, i6, i7, i8, i9;
			TRY(get_nbits(s, 4, &i4));
			dst->k[4] = lpc_k5_tab[i4]; // K5
			TRY(get_nbits(s, 4, &i5));
			dst->k[5] = lpc_k6_tab[i5]; // K6
			TRY(get_nbits(s, 4, &i6));
			dst->k[6] = lpc_k7_tab[i6]; // K7
			TRY(get_nbits(s, 3, &i7));
			dst->k[7] = lpc_k8_tab[i7]; // K8
			TRY(get_nbits(s, 3, &i8));
			dst->k[8] = lpc_k9_tab[i8]; // K9
			TRY(get_nbits(s, 3, &i9));
			dst->k[9] = lpc_k10_tab[i9]; // K10
		}
	}
...
static bool lpc_synth_preload(LPC_SYNTH* s, int *__res)
{
	static int magic;
	static int ret;
	crBegin;

	TRY(get_nbits(s, 8, &magic));
	...
	TRY(lpc_get_frame(s, &s->frame_prev, &s->frame_prev, &ret));
	TRY(lpc_get_frame(s, &s->frame_next, &s->frame_prev, &ret));
...
static bool lpc_synth_run(LPC_SYNTH* s, short* pcm_out, int* eos, short **__res)
{
	static int i;
	static int excit;
	static int ret;
...
void* lpc_d6_synth_new(int (*feed)(void*, unsigned char*), void* host, int variant)
{
	...
	lpc = (LPC_SYNTH*)mymalloc(0, sizeof(LPC_SYNTH));
	if (!lpc) return 0;
...
int lpc_output_upsample(LPC_SYNTH* s, short* output, const short* input, int in_len)
{
	const int out_len = (in_len * LPC_SRC_DENOMINATOR) / LPC_SRC_STEP;  // 计算输出长度

	int n = 0; // 输入样本整数位置

	for (int i = 0; i < out_len; i++) {
		// 计算分数部分（Q16格式）
		int frac = (s->src_rem * 65536) / LPC_SRC_DENOMINATOR;

		// 获取相邻样本（处理边界情况）
		short s0 = (n == 0) ? s->src_base_sample : input[n - 1];
		short s1 = input[n];

		// 线性插值计算（32位中间结果）
		int interpolated = s0 * (65536 - frac) + s1 * frac;
		interpolated >>= 16;  // 转换回Q0格式
		...
		// 更新相位状态
		s->src_rem += LPC_SRC_STEP;
		if (s->src_rem >= LPC_SRC_DENOMINATOR) {
			s->src_rem -= LPC_SRC_DENOMINATOR;
			n++;
		}
	}

	s->src_base_sample = input[in_len - 1];

	return out_len;
}

bool lpc_d6_synth_do(void* lpc, short* pcm, int* pcm_size, int* restart, int *__res)
{
	static LPC_SYNTH* s;

	static int eos;
#ifndef LPC_UPSAMPLE
	static short* pcm_base;
#endif
	crBegin;
...
#ifdef LPC_UPSAMPLE
	static short *_;
	TRY(lpc_synth_run(s, s->synth_buf, &eos, &_));
	if (!eos) {
		*pcm_size = lpc_output_upsample(s, pcm, s->synth_buf, LPC_SAMPLES_PER_FRAME);
	} else {
		*pcm_size = 0;
	}
#else
	pcm_base = pcm;
	TRY(lpc_synth_run(s, pcm, &eos, &pcm));
